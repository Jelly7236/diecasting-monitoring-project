...
def server_cause(input, output, session):
    ...
    def p_chart():
        ...
        if mold_sel in (None, ""):
            return ui.div("몰드를 선택하세요.", style="padding:12px; color:#6b7280;")
        mold = str(mold_sel)
        ...
        df = filt()
        if df.empty:
            fig = go.Figure(); fig.add_annotation(text="선택한 조건의 데이터가 없습니다.", showarrow=False)
            fig.update_layout(template="plotly_white", height=420)
            return fig_html(fig, 420)

        if mold == "전체":
            fault_df = fault_filt()
            if fault_df.empty or ("메시지" in fault_df.columns and len(fault_df.columns) == 1):
                return ui.div(
                    "선택한 기간에 불량 로그가 없어 전체 몰드 p-관리도를 그릴 수 없습니다.",
            ...
            if df.empty:
                return ui.div(
                    "선택한 기간의 불량 로그와 일치하는 품질 데이터가 없습니다.",
                    style="padding:12px; color:#6b7280;"
                )
        else:
        ...
        if mold == "전체":
            ...
            if not pd.notna(pbar):
                fig = go.Figure(); fig.add_annotation(text="전체 몰드 기준 p̄ 값이 계산되지 않았습니다.", showarrow=False)
                fig.update_layout(template="plotly_white", height=420)
                return fig_html(fig, 420)
            k3 = 3.0
            title_txt = f"전체 몰드 통합 p-관리도 (n={NI})"
            y_title = "p̂(최근 불량률)"
        else:
            ...
            title_txt = f"몰드 {mold} p-관리도 (n={NI})"
            y_title = f"p̂(구간 n={NI} 불량률)"
        ...
        fig.update_layout(template="plotly_white", height=420,
                          ...
                          xaxis_title="시간", yaxis_title=y_title,
    ...
    def shap_plot():
        ...
        fig.update_layout(
            ...
            title=f"SHAP 중요 변수 기여도 (평균 |SHAP|){' - 전체 몰드' if mold_sel == '전체' else f' - 몰드 {mold_sel}'}",
            xaxis_title="평균 |SHAP|",
            yaxis_title="변수",
            yaxis=dict(
                categoryorder="array",
    ...
    def fault_filt():
        ...
        if input.p_mold() not in (None, "전체"):
            if "mold_code" not in df.columns:
                return pd.DataFrame({"메시지": ["CSV에 몰드 코드가 없습니다."]})
    ...
    def var_rel_table():
        ...
        pretty = df_top[["rank","변수","Score","상태","shap","high","low"]].rename(columns={
            "rank":"Rank","변수":"변수","Score":"원인 기여 횟수","상태":"상태","shAP":"SHAP횟수","high":"HIGH횟수","low":"LOW횟수"
        })
        # 오타 방지 재정의
        pretty = df_top[["rank","변수","Score","상태","shap","high","low"]].rename(columns={
            "rank":"Rank","변수":"변수","Score":"원인 기여 횟수","상태":"상태","shap":"SHAP횟수","high":"HIGH횟수","low":"LOW횟수"
    ...
    def var_rel_bar():
        ...
        fig.add_trace(go.Bar(
            ...
            hovertemplate="%{x}<br>원인 기여 횟수: %{y}<extra></extra>",
            name=""
        ))
        # 색상 범례(LOW/HIGH)
