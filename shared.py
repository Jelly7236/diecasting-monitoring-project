...
### 단변량 관리도 ARIMA 모델 로드
ARIMA_INFO_PATH = data_dir / "residual_analysis_results.csv"

arima_models = {}

try:
    arima_info = pd.read_csv(ARIMA_INFO_PATH)
    print(f"📄 ARIMA 표준화 메타데이터 로드 완료: {len(arima_info)}행")
...
for _, row in arima_info.iterrows():
    ...
    try:
        ...
        arima_models[key] = {
            ...
            "n_residuals": int(row.get("n_residuals", 0)),
            ...
            "ucl_individual": float(row.get("UCL_individual", 0)),
            "lcl_individual": float(row.get("LCL_individual", 0)),
            "ucl_standardized": float(row.get("UCL_standardized", 0)),
            "lcl_standardized": float(row.get("LCL_standardized", 0)),
        }
...
# ====================================================
# 다변량 관리도용 공분산 행렬 자동 로드
# ====================================================
def load_cov_matrices(folder_path="./data/cov_matrices"):
    """
    df_covXXXX_Y.csv 파일 자동 로드
    예: df_cov8412_3.csv → mold=8412, process=3
    """
    cov_dict = {}
    import re, os
    import pandas as pd
    ...
    if not os.path.exists(folder_path):
        print(f"⚠️ 공분산 폴더 없음: {folder_path}")
        return cov_dict

    for fname in os.listdir(folder_path):
        if not fname.startswith("df_cov") or not fname.endswith(".csv"):
            continue

        try:
            # ✅ 정규식으로 mold_code와 process_num 추출
            match = re.match(r"df_cov(\d+)_(\d+)\.csv", fname)
            if not match:
                print(f"⚠️ 파일명 패턴 불일치: {fname}")
                continue

            mold_code = int(match.group(1))
            proc_num = int(match.group(2))

            df = pd.read_csv(os.path.join(folder_path, fname))
            df_numeric = df.drop(columns=["var"], errors="ignore")
            cov_matrix = df_numeric.to_numpy()

            cov_dict.setdefault(mold_code, {})[proc_num] = cov_matrix
            print(f"✅ 공분산 행렬 로드 완료: mold={mold_code}, process={proc_num}, shape={cov_matrix.shape}")
...
# 전역 등록
cov_matrices = load_cov_matrices()
